...
def get_cats(db: Session, user_id: int, skip: int = 0, limit: int = 100) -> list[models.Cat]:
    ...
    try:
        ...
        return db.query(models.Cat).filter(models.Cat.user_id
        ...
        logger.error(
            "Database error retrieving cats for user %d: %s",
            user_id,
            str(e))  # import logging
        db.rollback()
        return []
...
def update_cat(db: Session, cat_id: int, cat: schemas.CatCreate,
               user_id: int) -> Optional[models.Cat]:
...
def get_weight_records(db: Session, cat_id: int, skip: int = 0,
                       limit: int = 100) -> list[models.WeightRecord]:
...
def create_weight_record(db: Session,
                         weight_record: schemas.WeightRecordCreate,
                         cat_id: int) -> Optional[models.WeightRecord]:
...
def create_default_user(db: Session) -> Optional[models.User]:
    ...
    try:
        ...
        orphan_cats = db.query(models.Cat).filter(models.Cat.user_id is None).all()
